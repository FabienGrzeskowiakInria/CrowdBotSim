...
namespace VOG {
	...
	public class RVOQuadtree
	{
		...
		public void Query ( Vector2 p, float radius, Agent agent,bool group=false ) {
            if (!group)
            {
            ...
            {
                QueryRec4Group(0, p, radius, agent, bounds);
            }
		}

        float QueryRec4Group(int i, Vector2 p, float radius, Agent agent, Rect r)
        {
            if (nodes[i].child00 == i)
            {
                // Leaf node
                ...
                while (a != null)
                {
                    float v = agent.InsertGroupAgentNeighbour(a, radius * radius);
                    if (v < radius * radius)
                    {
                        radius = Mathf.Sqrt(v);
                    }
            ...
            else
            {
                ...
                if (p.x - radius < c.x)
                {
                    if (p.y - radius < c.y)
                    {
                        radius = QueryRec4Group(nodes[i].child00, p, radius, agent, Rect.MinMaxRect(r.xMin, r.yMin, c.x, c.y));
                    }
                    if (p.y + radius > c.y)
                    {
                        radius = QueryRec4Group(nodes[i].child01, p, radius, agent, Rect.MinMaxRect(r.xMin, c.y, c.x, r.yMax));
                    }
                }
                ...
                if (p.x + radius > c.x)
                {
                    if (p.y - radius < c.y)
                    {
                        radius = QueryRec4Group(nodes[i].child10, p, radius, agent, Rect.MinMaxRect(c.x, r.yMin, r.xMax, c.y));
                    }
                    if (p.y + radius > c.y)
                    {
                        radius = QueryRec4Group(nodes[i].child11, p, radius, agent, Rect.MinMaxRect(c.x, c.y, r.xMax, r.yMax));
                    }
                }
            }
