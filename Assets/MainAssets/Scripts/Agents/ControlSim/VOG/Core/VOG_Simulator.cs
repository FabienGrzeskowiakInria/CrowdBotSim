...
namespace VOG
{
	...
	public interface IAgent {
		 ...
		 * This is the recommended way of reading the position of an agent.
		 * Will be interpolated if the interpolation setting is enabled on the simulator.
		 */
		Vector3 InterpolatedPosition {get;}
		
		/** Position of the agent.
		 * This cannot be changed manually. Use the Teleport method if you want to change the position of the agent, note that this should not be done
		 * during normal navigation since doing it effectively disables local avoidance.
		 * \see InterpolatedPosition
		 */
		Vector3 Position {get;}

		/** Sets the Y coordinate of the agent only.
		 * This will not interfere with local avoidance like when using the #Teleport method so this is the recommended way to
		 * adjust the position of the agent based on e.g how the ground looks.
		 */
	...
	public class Simulator {
        ...
        /// FORGROUP///////////////////
        /// </summary>
        public float[][] relationMatrix; //The relation matrix used in simulation
        public bool simulateGroup = false;
        public bool usingRVO1 = false;
        public bool usingRVO2 = false;
        /// <summary>
        /// FOR STATISTIC
        /// </summary>
		 ...
		 * \see RemoveObstacle
		 */
		public ObstacleVertex AddObstacle (Vector3[] vertices, float height, Matrix4x4 matrix, VOGLayer layer = VOGLayer.DefaultObstacle) {
        ...
        public List<List<Vector2>> agentPosList = new List<List<Vector2>>();

        private void createRelationMatrix()
        {
            //Create Matrix
            int aNum=agents.Count;
            relationMatrix = new float[aNum][];
            for (int i = 0; i < aNum; i++)
            {
                relationMatrix[i] = new float[aNum];
                for (int j = 0; j < aNum; j++)
                {
                    if (i == j || agents[i].groupID<0)
                        relationMatrix[i][j] = 0f;
                    else
                        relationMatrix[i][j] = agents[i].groupID==agents[j].groupID? Random.Range(0.1f, 1f) : 0; 
                }
            }
        }
        ...
        public void Update (float dTime) {
            ...
            if (simulateGroup && relationMatrix == null)
            {
			...
			if (lastStep < 0) {
				lastStep = currTime - DesiredDeltaTime;
				deltaTime = DesiredDeltaTime;
				prevDeltaTime = deltaTime;
				lastStepInterpolationReference = lastStep;
			}

            if (forAnalysis || currTime - lastStep >= DesiredDeltaTime) 
            {

				//for (int i=0;i<agents.Count;i++) {
    //                    agents[i].Interpolate(dTime);
    //            }

				lastStepInterpolationReference = currTime;

				prevDeltaTime = DeltaTime;
				deltaTime = currTime - lastStep;
				lastStep = currTime;
				...
				if (Multithreading) {
					...
					for (int i=0;i<agents.Count;i++) {
                       //if (forAnalysis)
                        {
                            agents[i].forAnalysis = forAnalysis;
                            agents[i].unitTime = stepTime;
                        }
                        agents[i].Update ();
                    ...
                    if (calculateStatistics)
                    {
                        ...
                        for (int i = 0; i < agents.Count; i++)
                        {
                            averageDist += agents[i].averageDist;
                        }
                        if(agents.Count>0)
                        averageDist /= agents.Count;
                    }
		...
		internal class WorkerContext {
            ...
            public Agent.VG[] vgs = new Agent.VG[20];

			public const int KeepCount = 2;//origin 3
            ...
            public float[] bestScores_G = new float[KeepCount + 1];
